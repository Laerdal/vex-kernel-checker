...
from .common import timed_method, VulnerabilityAnalysis, VulnerabilityState, Justification, Response
...
class ConfigurationAnalyzer(VexKernelCheckerBase):
    ...
    def in_kernel_config(self, config_options: Set[str], kernel_config: List[str]) -> VulnerabilityAnalysis:
        ...
        if enabled_required or enabled_optional:
            ...
            detail_parts = []
            if enabled_required:
                detail_parts.append(f"Required configs enabled: {', '.join(sorted(enabled_required))}")
            if enabled_optional:
                detail_parts.append(f"Optional configs enabled: {', '.join(sorted(enabled_optional))}")
    ...
    def find_config_options_for_file(self, source_file_path: str, kernel_source_path: str) -> Set[str]:
        """
        ...
        try:
            ...
            makefile_configs = self.find_makefiles_config_options(source_file_path, kernel_source_path)
            config_options.update(makefile_configs)
            ...
            if self.verbose and config_options:
                print(f"Found {len(config_options)} config options for {source_file_path}: {', '.join(sorted(config_options))}")
