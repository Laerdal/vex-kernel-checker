...
from .common import VulnerabilityState, timed_method
...
class ReportGenerator(VexKernelCheckerBase):
    ...
    def _generate_recommendations(self, report: Dict) -> List[str]:
        ...
        if report["analysis_coverage"] < 80:
            recommendations.append(
                "Improve analysis coverage by providing kernel source and configuration"
            )
        ...
        if critical_high > 0:
            recommendations.append(
                f"Prioritize {critical_high} CRITICAL/HIGH severity vulnerabilities"
            )
    ...
    def generate_detailed_report(
        self, vex_data: Dict, include_configs: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        for vuln in vulnerabilities:
            ...
            if "response" in analysis:
                detailed_vuln["analysis"]["response"] = (
                    [analysis["response"]]
    ...
    def _generate_performance_recommendations(self) -> List[str]:
        ...
        for cache_type, hit_rate in hit_rates.items():
            if hit_rate < 50:
                recommendations.append(
                    f"Consider optimizing {cache_type} cache usage (hit rate: {hit_rate:.1f}%)"
                )
