...
from .common import timed_method, check_interrupt


class PatchManager(VexKernelCheckerBase):
    """Manages patch fetching and processing from various sources."""
    
    def __init__(self, verbose: bool = False, edge_driver_path: Optional[str] = None, 
        ...
        if self.verbose:
            ...
            print(f"Patch Manager initialized - Selenium: {selenium_status}, WebDriver: {driver_status}")
        ...
        for line in patch_info.split('\n'):
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if clean_path and (clean_path.endswith('.c') or clean_path.endswith('.h') or 'Makefile' in clean_path):
    ...
    def extract_patch_url(self, cve_info) -> Optional[str]:
        ...
        if self.verbose:
            print(f"🔍 PatchManager: extract_patch_url called")
        
        if not cve_info.patch_urls:
            if self.verbose:
                print(f"🔍 PatchManager: No patch URLs found in CVE info")
            return None
        
        if self.verbose:
            print(f"🔍 PatchManager: Found {len(cve_info.patch_urls)} patch URLs: {cve_info.patch_urls}")
        ...
        if self.verbose:
            print(f"🔍 PatchManager: All URLs were ignored, returning None")
        return None
